...
const HistoryModal: React.FC<HistoryModalProps> = ({ isOpen, onClose, onLoadEntry }) => {
  ...
  const filteredHistory = history.filter(entry => {
    const matchesText = entry.title.toLowerCase().includes(filterText.toLowerCase()) || 
                        entry.tags.some(t => t.toLowerCase().includes(filterText.toLowerCase())) ||
                        entry.instrument_estimate.toLowerCase().includes(filterText.toLowerCase());
    const matchesSource = filterSource === 'all' || entry.source_type === filterSource;
    const matchesConf = entry.avg_confidence >= filterMinConfidence;
    const matchesBPM = entry.bpm_detected >= filterBPM[0] && entry.bpm_detected <= filterBPM[1];
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                        <h4 className="text-sm font-medium text-zinc-200 truncate pr-2">{entry.title}</h4>
                      ...
                      <div className="mt-2 h-1 w-full bg-zinc-800 rounded-full overflow-hidden">
                        <div 
                            ...
                            style={{ width: `${entry.avg_confidence * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="max-w-3xl mx-auto space-y-8">
                    ...
                    <div className="space-y-2 flex-1">
                        {isRenaming ? (
                            ...
                            <div className="flex items-center gap-2 group">
                                <h1 className="text-3xl font-bold text-white tracking-tight">{selectedEntry.title}</h1>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800/60 shadow-sm relative overflow-hidden group">
                        ...
                        <div className="text-2xl font-bold text-white group-hover:text-indigo-400 transition-colors">{(selectedEntry.avg_confidence * 100).toFixed(0)}<span className="text-sm text-zinc-600">%</span></div>
                    </div>
                    <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800/60 shadow-sm relative overflow-hidden">
                        ...
                        <div className="text-2xl font-bold text-white">{selectedEntry.bpm_detected}</div>
                    </div>
                    <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800/60 shadow-sm relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500/50"></div>
                        <div className="text-xs text-zinc-500 uppercase font-bold mb-1">Note Count</div>
                        <div className="text-2xl font-bold text-white">{selectedEntry.notes_count}</div>
                    </div>
                    <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800/60 shadow-sm relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="bg-zinc-950 rounded-lg border border-zinc-800 p-5 space-y-3">
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <span className="font-mono text-zinc-300">{selectedEntry.user_edits.notes_modified}</span>
                            </div>
                            <div className="w-full h-px bg-zinc-900"></div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-zinc-500">Notes Added</span>
                                <span className="font-mono text-green-400">+{selectedEntry.user_edits.notes_added}</span>
                            </div>
                            <div className="w-full h-px bg-zinc-900"></div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-zinc-500">Notes Deleted</span>
                                <span className="font-mono text-red-400">-{selectedEntry.user_edits.notes_deleted}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {Object.entries(selectedEntry.exports).map(([key, val]) => (
